...
AudioProcessorValueTreeState::ParameterLayout PolyWaveSynthAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<AudioParameterInt>("osc_oscType", "OscType", 0, 4, 0)); // from 3 to 4 in order to have different tables
...
void PolyWaveSynthAudioProcessor::addParameterListeners()
{
    ...
    parameters.addParameterListener("osc_oscType", this); //addParameterListener("id", potAmplitude)
...
void PolyWaveSynthAudioProcessor::updateOsc()
{
    ...
    // if type is 4, custom table needs to be generated
   /* if (*osc_oscType == 4) {
        generator.generateCustomTable();
    }*/ // this shit breaks oscillator after selecting type, maybe move this fucker to synthEngine
...
void PolyWaveSynthAudioProcessor::updateFilterEnv() 
{
    ...
    synthEngine.setFilterADSR(*filterEnv_attack, *filterEnv_decay, *filterEnv_sustain, *filterEnv_release); //alv
}
